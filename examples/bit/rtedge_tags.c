...
static const char *const k_bit_rtedge_scalar_specs[] = {
	"DWORD#dw1",
	"DWORD#dw2",
	"DWORD#dw3",
};
...
void IlRtedgeTags_Init(void)
{
	...
	for (i = 0; i < sizeof(k_bit_rtedge_scalar_specs) / sizeof(k_bit_rtedge_scalar_specs[0]); i++)
		il_rtedge_apply_instruction_spec(k_bit_rtedge_scalar_specs[i]);
