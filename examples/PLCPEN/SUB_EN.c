...
PLCPEN_API void SUB_EN(PVOID instance)
{
	struct SUB_PIN
	{
		T_BOOL		EN;			// 実行フラグ
		...
		P_ANY		IN2;        // 入力2
		T_BOOL		ENO;		// 実行フラグ(出力)
		P_ANY		OUT;		// IN1とIN2の減算値
	} *pin = (struct SUB_PIN*)instance;

	// 実行フラグONの場合
	if (pin->EN)
	{
		SUB(pin->IN1, pin->IN2, NULL, NULL);	// ADD呼出

		if (pin->ENO = GetSubEno())	// SUB実行結果ENO設定
		{
			switch (pin->IN1->Type)
			{
				// データ型に応じて結果をキャストして格納
...
/// <param name="IN1"></param>
/// <param name="IN2"></param>
PLCPEN_API void SUB_T_T_EN(PVOID instance)
{
	struct ADD_PIN
	{
		T_BOOL		EN;			// 実行フラグ
	...
	if (pin->EN)
	{
		SUB_T_T(pin->IN1, pin->IN2, NULL, NULL);	// SUB_T_T呼出

		if (pin->ENO = GetSubEno())	// SUN_T_T実行結果ENO設定
		{
