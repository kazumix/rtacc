...
## 4. .c / .obj / .o / .lib / .st / .il の扱い

- `.c` :  
  - LLVM ツールセットでは `.c → .ll → .asm → .obj` を生成  
  - VS ツールセットでは `cl` で直接 `.obj` を生成
- `.st` (Structured Text) :  
  - `llst.exe` で `.ll` に変換後、clang で .asm → .obj。llst の探索順: **LLST** → **RTACC_BIN** → rtacc と同じ dir → projectDir → PATH
- `.il` (Instruction List) :  
  - `llil.exe` で `.ll` に変換後、clang で .asm → .obj。llil の探索順: **LLIL** → **RTACC_BIN** → rtacc と同じ dir → projectDir → PATH
- `.obj` / `.o` :  
...
**rtacc / llil / llst の配置**  
一式を 1 フォルダ（例: `bin`）にまとめ、**そのフォルダを PATH に追加**するのがおすすめです。どのウィンドウ・IDE から実行しても llil/llst が PATH から見つかります。環境変数 **RTACC_BIN** でも指定できますが、別ウィンドウで有効にしづらい場合は PATH の方が確実です。サンプル（examples）には exe を同梱していません。
...
| `LLVM` | LLVM の bin ディレクトリ（既定: `C:\Program Files\LLVM\bin`） |
| `RTACC_BIN` | rtacc / llil / llst を置いたディレクトリ。.st / .il ビルド時の llst / llil 探索に使う |
| `LLST` | llst.exe のフルパス（ST 用。未設定時は RTACC_BIN → rtacc 同 dir → projectDir → PATH） |
| `LLIL` | llil.exe のフルパス（IL 用。未設定時は RTACC_BIN → rtacc 同 dir → projectDir → PATH） |

**環境変数が効かない場合**  
RTACC_BIN を追加しても別ウィンドウや IDE から効かない場合は、**bin フォルダを PATH に追加**してください。rtacc は llil/llst を最後に PATH からも探すため、PATH に bin を入れておけばどのプロセスからでも見つかります。
