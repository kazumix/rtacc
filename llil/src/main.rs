...
/// IL の即値かどうか判定し、整数ならその値を返す。
/// INT# / UINT# / DWORD# は いずれも i32（LLVM IR 上）として扱う。
/// 例: INT#5 -> Some(5), UINT#100 -> Some(100), DWORD#1 -> Some(1)
fn parse_int_literal(operand: &str) -> Option<i64> {
    let s = operand.trim();
    let (prefix, rest) = s.split_once('#')?;
    let prefix = prefix.to_ascii_uppercase();
    if prefix != "INT" && prefix != "UINT" && prefix != "DWORD" {
        return None;
    }
    rest.parse::<i64>().ok()
}

/// 即値（INT#/UINT#/DWORD#5 など）なら true。変数として扱うべきでない。
...
/// `--memory=rtedge` 用: VAR 由来の `@il_slot_*` を Eg Entry に張る C を生成（手書き rtedge_tags / slots_bind を不要にする）。
fn emit_rtedge_glue_c(vars: &[VarDecl], pou_name: &str) -> String {
    let mut s = String::new();
    s.push_str(
        "/* Generated by llil: rtedge glue (IlRtedgeTags_Init + IlRtedgeSlots_BindEgEntry). Do not edit. */\n\n",
    );
    // 互換: IL_USE_ETAG_RTEDGE だけ指定されていても、この glue.c 内で EgApi 経路が成立するようにする。
    s.push_str("#if defined(IL_USE_ETAG_RTEDGE)\n");
    s.push_str("#define IL_USE_PLCP_RTEDGE 1\n");
    s.push_str("#define IL_USE_EGAPI 1\n");
    s.push_str("#endif\n\n");

    // EgApi 実処理を有効化する「1つの状態」を畳み込む。
    // - IL_USE_PLCP_RTEDGE / IL_USE_EGAPI が揃っている
    // - IL_EGAPI_STUB が有効でない（本番経路）
    s.push_str("#ifndef IL_EGAPI_PLCP_RTEDGE\n");
    s.push_str("#if defined(IL_USE_PLCP_RTEDGE) && defined(IL_USE_EGAPI) && !(defined(IL_EGAPI_STUB) && IL_EGAPI_STUB)\n");
    s.push_str("#define IL_EGAPI_PLCP_RTEDGE 1\n");
    s.push_str("#else\n");
    s.push_str("#define IL_EGAPI_PLCP_RTEDGE 0\n");
    s.push_str("#endif\n");
    s.push_str("#endif\n\n");

    s.push_str("#if IL_EGAPI_PLCP_RTEDGE\n\n");
    s.push_str("#include <egAPI.h>\n");
    s.push_str("#include <stddef.h>\n");
    s.push_str("#include <stdint.h>\n\n");
    s.push_str("#ifndef EDGE_SUCCESS\n#define EDGE_SUCCESS 0\n#endif\n\n");
    s.push_str("#ifndef EgVtSegment\n#define EgVtSegment 13\n#endif\n\n");
    s.push_str("#else\n\n#include <stddef.h>\n#include <stdint.h>\n\n#endif\n\n");
    s.push_str("#define IL_SLOT_EXT(name) extern void *il_slot_##name\n");
    s.push_str("#define IL_BIND(name) { #name, (void **)&il_slot_##name }\n\n");
    for v in vars {
        let ir = sanitize_llvm_name(&v.name);
        s.push_str(&format!("IL_SLOT_EXT({});\n", ir));
    }
    s.push('\n');
    s.push_str(&format!("extern void {}_slots_init(void);\n\n", pou_name));
    s.push_str("#if IL_EGAPI_PLCP_RTEDGE\n\n");
    s.push_str("static void *il_rtglue_dataptr_from_tagdesc(TagsDesc *td)\n{\n");
    s.push_str("\tuint16_t vt = td->var.vt;\n\n");
    s.push_str("\tif (vt == EgVtSegment || td->SHM.InUse == 1)\n");
    s.push_str("\t\treturn (void *)(uintptr_t)td->var.pSegment;\n");
    s.push_str("\treturn (void *)&td->var.val;\n");
    s.push_str("}\n\n");
    s.push_str("#endif\n\n");
    s.push_str("static const struct {\n\tconst char *tag;\n\tvoid **pstore;\n} il_slot_binds[] = {\n");
    for v in vars {
        let ir = sanitize_llvm_name(&v.name);
        s.push_str(&format!("\tIL_BIND({}),\n", ir));
    }
    s.push_str("};\n\n");
    s.push_str("void IlRtedgeSlots_BindEgEntry(void)\n{\n");
    s.push_str("#if IL_EGAPI_PLCP_RTEDGE\n");
    s.push_str("\tsize_t i;\n\n");
    s.push_str("\tfor (i = 0; i < sizeof(il_slot_binds) / sizeof(il_slot_binds[0]); i++) {\n");
    s.push_str("\t\tTagsDesc *td = NULL;\n");
    s.push_str("\t\tconst char *tag = il_slot_binds[i].tag;\n\n");
    s.push_str("\t\tif (EgTagGetProperty(tag, \"Entry\", &td, sizeof(td)) != EDGE_SUCCESS || td == NULL)\n");
    s.push_str("\t\t\tcontinue;\n");
    s.push_str("\t\t*il_slot_binds[i].pstore = il_rtglue_dataptr_from_tagdesc(td);\n");
    s.push_str("\t}\n");
    s.push_str("#else\n");
    s.push_str("\t(void)il_slot_binds;\n");
    s.push_str("#endif\n");
    s.push_str("}\n\n");
    s.push_str("void IlRtedgeTags_Init(void)\n{\n");
    s.push_str(&format!("\t{}_slots_init();\n", pou_name));
    s.push_str("#if IL_EGAPI_PLCP_RTEDGE\n");
    s.push_str("\tIlRtedgeSlots_BindEgEntry();\n");
    s.push_str("#endif\n");
    s.push_str("}\n");
    s
}
...
fn compile_il_to_llvm_ir(
    ...
    let rtedge_glue_c = if memory == MemoryKind::Rtedge {
        Some(emit_rtedge_glue_c(&vars, &pou_name))
    } else {
        None
    };

    Ok((m.to_string(), rtedge_glue_c))
}
...
fn usage(program: &str) {
    ...
    eprintln!("-o 指定時は rtedge のとき追加で <出力ベース名>_rtedge_glue.c を出力（IlRtedgeTags_Init / IlRtedgeSlots_BindEgEntry）。");
}
...
fn main() {
    ...
    let (ir, rtedge_glue_c) = match compile_il_to_llvm_ir(&prog, entry_name, memory) {
        Ok(x) => x,
        Err(e) => {
            eprintln!("コンパイルエラー: {e}");
            std::process::exit(1);
    ...
    //   rtedge かつ -o 指定時は同じディレクトリに <stem>_rtedge_glue.c も出力
    if let Some(out_path) = out_path {
        ...
        if let Some(glue) = rtedge_glue_c {
            ...
            let glue_path = p
                .parent()
                .unwrap_or(Path::new(""))
                .join(format!("{}_rtedge_glue.c", stem));
            if let Err(e) = fs::write(&glue_path, glue) {
                eprintln!("rtedge glue の書き込みに失敗しました: {e}");
                std::process::exit(1);
            }
        }
    } else {
